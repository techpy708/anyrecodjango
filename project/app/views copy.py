...
def process_data(request):
    ...
    def apply_header_style(workbook, sheet_name, title, blue_color):
    ...
    def saving_and_color_reco_sheet(workbook, sheet_name, title, blue_color, file1_cols, file2_cols, selected_f1=None, selected_f2=None,file1_name="File 1", file2_name="File 2"):
        if sheet_name not in workbook.sheetnames: return
        sheet = workbook[sheet_name]
    ...
    print(unique_key_str)
    
    existing_history_record = None 

    if not has_unlimited:
        # Call the helper function
        # Ensure 'check_license_validity' is imported or defined above!
        is_valid, result = check_license_validity(
            user, 
            original_file1, 
            original_file2, 
            len(df_file1), 
        ...
        # If invalid, 'result' is the error JsonResponse -> Return it immediately
        if not is_valid:
            return result
        
        # If valid, 'result' is the existing_history_record (or None)
            ...
            try:
                ...
                if user_sheet_name in workbook.sheetnames:
                    saving_and_color_reco_sheet(
                        ...
                        selected_f1=selected_f1_cols, selected_f2=selected_f2_cols,file1_name=original_file1_headername, file2_name=original_file2_headername
                    )
                ...
                for s_name, title in extra_sheets.items():
                    apply_header_style(workbook, s_name, title, '000c66')
    ...
    try:
        ...
        if existing_history_record:
            existing_history_record.created_at = timezone.now()
            existing_history_record.save()
        else:
    ...
    if request.method == 'POST' and request.FILES.getlist('pdf_files'):
        ...
        if not has_unlimited:
            ...
            try:
                ...
                # Check 3: Remaining Sets (Credit Limit)
                # We use .count() here assuming 1 credit = 1 batch processing event
                credits_used = ProcessingHistory.objects.filter(group=group).count()
        ...
        # Buffer to store the final ZIP file
        zip_buffer = io.BytesIO()
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            for uploaded_file in files:
                try:
                    ...
                    # Open the uploaded file directly from memory
                    with pdfplumber.open(uploaded_file) as pdf:
                        for page in pdf.pages:
                            ...
                            for table in tables:
                                ...
                                # Logic from your script: Skip empty or <= 6 columns
                                if df.empty or df.shape[1] <= 5:
                                    continue
                                
                                # Logic: Use first row as header if numeric (from your script)
                    ...
                    if valid_tables:
                        ...
                        # Logic: Set first row as header and remove duplicates
                        ...
                        # We use an in-memory buffer for the Excel file
                        excel_buffer = io.BytesIO()
                        
                        with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                            combined_df.to_excel(writer, index=False, sheet_name='Sheet1')
                            
                            # Apply the Blue Header styling from your script
                            workbook = writer.book
                        ...
                        # Write Excel buffer to Zip
                        original_name = os.path.splitext(uploaded_file.name)[0]
                        zip_file.writestr(f"{original_name}.xlsx", excel_buffer.getvalue())

                except Exception as e:
                    print(f"Error processing {uploaded_file.name}: {e}")
                    # You might want to add a text file to the zip explaining the error
